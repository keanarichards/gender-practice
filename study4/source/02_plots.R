...
dat <- dplyr::select(clean_fraud_removed,gender,comp_choice, practice_problems_binary, condition)  %>% filter(condition == "pract") %>% na.omit(practice_problems_binary) %>% 
...
dat1 <- dplyr::select(clean_fraud_removed,gender,comp_choice, practice_problems_binary, condition) %>% filter(condition == "pract")%>% na.omit(practice_problems_binary) %>% 
...
p <- ggplot(data = dat, aes(x = perc_gender_comp, fill = perc_gender_comp))  + 
  labs(x = 'Option selected', y = 'Percentage of participants who selected response option')  + scale_y_continuous(limits = c(0, 100))+theme(legend.position = "none") + guides (fill = "none")+
  geom_bar(aes(y = percent*100), position = "dodge", stat = "identity") +scale_fill_manual(values=c("springgreen3", "slateblue1", "orangered"))+ geom_errorbar(aes(ymin =(percent*100)-(error*100), ymax =(percent*100)+(error*100)), width=.05,
                                                                                                                                                  position=position_dodge(.9)) + theme_apa() + scale_x_discrete(labels = c("Men compete more", "Women compete more", "No difference in competitiveness"))+ theme(panel.border  = element_blank()) +
  #draws x and y axis line
  theme(axis.line = element_line(color = 'black')) +
  geom_hline(yintercept = 50, linetype = "dashed") 
