...
## for questions about perc_gender_comp_F & perc_gender_comp_M, given study design, will just draw from prev lit
## so correct answer will be "Men would choose tournament more often than piece rate" (perc_gender_comp_M) & "Women would choose piece rate more often than tournament" (perc_gender_comp_F)

# Do you think men or women spent more time practicing/studying before multiplication task? 

## women practice the task more 
...
clean <- clean %>% 
  mutate(percent_rank = percent_rank(task_score), conf_rank_decimal = clean$conf_rank/100) %>% drop_na(percent_rank)
...
## shuffling order to make pairing up truly random 
clean <-clean[sample(1:nrow(clean), replace = F), ]
clean$bonus_assn <- 0

clean$bonus_assn <- rep(1:5, length.out = nrow(clean))

total_payment <- clean 

## conf_accuracy q == 1; perc_task_gender_pract == 2; better_gender_guess == 3; perc_gender_comp_M == 4; perc_gender_comp_F == 5
## "correct" ==1, "Women were more likely to practice/study than men" == 2; "Men" == 3; 
## "Men would choose tournament more often than piece rate" == 4; "Women would choose piece rate more often than tournament" == 5

total_payment$bonus_qs <- ifelse(total_payment$conf_accuracy == "correct" & total_payment$bonus_assn == 1, .1, 0)
total_payment$bonus_qs <- ifelse(total_payment$perc_task_gender_pract == "Women were more likely to practice/study than men" & total_payment$bonus_assn == 2, .1, total_payment$bonus_qs)
total_payment$bonus_qs <- ifelse(total_payment$better_gender_guess == "Men" & total_payment$bonus_assn == 3, .1, total_payment$bonus_qs)
total_payment$bonus_qs <- ifelse(total_payment$perc_gender_comp_M == "Men would choose tournament more often than piece rate" & total_payment$bonus_assn == 4, .1, total_payment$bonus_qs)
total_payment$bonus_qs <- ifelse(total_payment$perc_gender_comp_F == "Women would choose piece rate more often than tournament" & total_payment$bonus_assn == 5, .1, total_payment$bonus_qs)
...
### filtered based on piece rate assn
PR_choice <- subset(clean, condition == "piecerate", select = c(workerId, task_score))
...
tourn_choice <- subset(clean, condition == "tournament", select = c(workerId, task_score))
...
## they lose tournament: opponent ID = A19CA1G04F23NN; score = 13, do not need to add as row to tourn_winners dataset

tourn_winners <- tourn_choice_even %>% filter(W_L == "win"| W_L == "tie") 
...
total_payment <- left_join(total_payment, PR_choice,by = "workerId")
